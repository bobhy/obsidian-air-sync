...
## Settings
- "Remote Vault Folder"  
  The name of the folder under `obsidian-air-sync/` in Google Drive. Defaults to a sanitized form
  of the local Obsidian vault name (non-alphanumeric characters except `-` replaced with `_`).  
  The user may enter a custom name while the plugin is **disconnected** from Google Drive; the field
  is read-only while connected. Changing the name clears local sync history so the next sync is a
  full scan against the newly targeted folder. The custom name is preserved across
  disconnect/reconnect cycles.  
  Use cases:
    - Two vaults with different local names sharing one Drive folder (e.g. for testing or migration).
    - Keeping the Drive folder name stable after renaming the local vault.  
  If the user manually renames the Drive folder, the plugin will create a new folder under the
  effective name on the next connect.
